...
import com.emp.ConvertModelToBean;
...
//        });
    ...
    @GetMapping(value = "/innerJoin/{deptId}")
    public ResponseEntity innerJoin(@PathVariable long deptId){
        List<DeptEmp1> employees = employeeService.getEmployeeListByInnerJoin(deptId);
//        .stream().map(employee -> {
//            LOGGER.info(employee.getManager());
//            int empId = Integer.parseInt(employee.getManager());
//            if(empId != 0)
